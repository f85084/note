...
function mUrl($uid){
	$re='';
	if(!empty($uid)){
		$re='http://www.etidol.com/article.php?uid='.$uid.'&ftoken='.base64_encode(authcode(rand(0,99).'.'.$uid.'.'.rand(0,99).'.'.(time()+30*86400)));	
	}
	return $re;
}
/*編輯者*/
...
/*編輯項目*/
$queryl = $db->query("SELECT id,name from b_s_group");
$lAdmin=array();
        ...
        <form name="form1" id="form1" action="" enctype="multipart/form-data" method="get">
            <table cellpadding="0" cellspacing="0" class="menutable" height="100%">
                ...
                <tr>
                    ...
                    <td width="150" align="center">編輯ID</td>
                ...
                <tr>
                    ...
                    <td colspan="5"><input type="radio" name="ml" id="ml" value="" <?=empty($_GET['ml'])? ' checked="checked"': '';?> />不拘
                        <? $x=1;foreach($lAdmin as $k => $v){?><input type="radio" name="ml" id="ml"  value="<?=$k?>" <?=($k==$_GET['ml'])? ' checked="checked"': '';?> />
                            <?=$v;?>
                                <?=($x%10==0)?'<br>':'';?>
                                    <? ++$x;}?>
                    </td>
